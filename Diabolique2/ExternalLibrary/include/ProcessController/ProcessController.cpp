...
namespace ProcessController
{
	...
	inline decltype(&SetStdHandle) _SetStdHandle;
	inline decltype(&PeekNamedPipe) _PeekNamedPipe;
	inline bool bInited = false;
	...
	)
	{
		...
		_SetStdHandle = (decltype(&SetStdHandle))(pSetSTDHandle);
		_PeekNamedPipe = (decltype(&PeekNamedPipe))(pPeekNamedPipe);
		bInited = true;
	}
	...
	bool CProcess::RecvDataFromProcess(void* pData, size_t nBufferSize, size_t* pnDataRead)
	{
		*pnDataRead = 0;
		...
		if (!_PeekNamedPipe(m_hSTDOUTRead, NULL, 0, NULL, &dwBytesAvail, NULL))
			return false;
		
		if(dwBytesAvail)
			return _ReadFile(m_hSTDOUTRead, pData, nBufferSize, (LPDWORD)pnDataRead, NULL);

		return true; // this isn't implicitly a failure!
	}


	bool CProcess::RedirectHandles()
	{
		if (!_SetStdHandle(STD_INPUT_HANDLE, m_hSTDINRead))
			return false;

		if (!_SetStdHandle(STD_OUTPUT_HANDLE, m_hSTDOUTWrite))
			return false;

		if (!_SetStdHandle(STD_ERROR_HANDLE, m_hSTDOUTWrite))
			return false;
