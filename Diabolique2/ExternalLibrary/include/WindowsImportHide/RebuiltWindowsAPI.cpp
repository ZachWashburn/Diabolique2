...
namespace RebuiltWindowsAPI {
	...
	PVOID WINAPI ImageDirectoryEntryToDataRom(PVOID Base, WORD HeaderMagic, USHORT DirectoryEntry, ULONG* Size, IMAGE_FILE_HEADER* ImageFileHeader, IMAGE_ROM_OPTIONAL_HEADER* ImageRomHeaders)
	{
		*(ULONG*)Size = NULL;
		...
		{
			if (DirectoryEntry == 3 && _stricmp((char*)pSectionHeader->Name, XorStr(".pdata")) == NULL)
				break;
			if (DirectoryEntry == 6 && _stricmp((char*)pSectionHeader->Name, XorStr(".rdata")) == NULL)
			{
				*(ULONG*)Size = NULL;
	...
	std::string find_file_path_recursive(std::string fileName, std::string szPath)
	{
		...
		for (;;)
		{
			try {
				for (; i != end; ++i)
					if (!is_directory(i->path())) // lstrcmpA
						if (!_strcmpi(i->path().filename().string().c_str(), filename.filename().string().c_str()))
